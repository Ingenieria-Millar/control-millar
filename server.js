...
/* ════════════════════════════════════════════════════════════
   RESPONSIVE ingresos.html — complemento
   ════════════════════════════════════════════════════════════ */
...
@media (min-width: 1600px) {
  html { font-size: 18px; }
  .ia-reg-grid { grid-template-columns: repeat(4, 1fr); }
}
@media (min-width: 2000px) {
  html { font-size: 20px; }
  .ia-reg-grid { grid-template-columns: repeat(5, 1fr); }
}

/* Fix scroll en select nativo en móvil */
#ia-emp-area {
  touch-action: pan-y;
  overscroll-behavior: contain;
  -webkit-overflow-scrolling: touch;
}
/* Aumentar tamaño mínimo del select en móvil para mejor UX */
...
/* ── TOPBAR UNIFICADO — igual al Tablero CI ── */
.ia-header, .nav-bar, nav.nav-bar {
  position: sticky; top: 0; z-index: 50; flex-shrink: 0;
  display: flex; align-items: center; justify-content: space-between;
  padding: 0 clamp(12px,2.5vw,24px); gap: 8px; min-height: 64px;
  background: #fff;
...
/* Raya degradada ABAJO */
.ia-header::after, .nav-bar::after {
  content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 3px;
  background: linear-gradient(90deg,#1e3a8a,#2563eb,#60a5fa);
  top: auto;
}
/* Badge usuario — igual al CI */
.ia-sup-badge, .hd-user-badge {
...
/* Dot verde */
.ia-sup-dot, .hd-user-dot {
  width: 9px; height: 9px; border-radius: 50%;
  background: #22c55e; box-shadow: 0 0 6px rgba(34,197,94,.7);
  flex-shrink: 0;
}
/* Botón volver */
.ia-header .btn-volver, .nav-bar .btn-volver, .hd-back-btn {
  padding: 6px 14px; border-radius: 8px;
  border: 1.5px solid #e2e8f0; background: #fff;
  color: #1e3a8a; font-family: 'Nunito', sans-serif;
  font-size: 12px; font-weight: 700; cursor: pointer;
  transition: all .2s; white-space: nowrap;
}
.ia-header .btn-volver:hover, .hd-back-btn:hover {
  background: #f1f5f9; border-color: #93c5fd;
}
</style>
</head>
<body>
  ...
  ws.onmessage = (e) => {
    try{
      ...
      if(msg.type === 'init'){
        ...
        // Si la app ya arrancó (login visible), actualizar el select con datos reales
        if(_appArranged){
...
// ── Arrancar inmediatamente sin esperar WS ──────────────────────
// El WS sincronizará datos en background cuando conecte.
// Así no queda trabado si Render está durmiendo.
...
// ── Init — cargar config y arrancar inmediatamente ──────────────
loadConfigFromServer(); // no bloquea
connectWS();            // WS sincroniza datos en background
arrancarApp();          // mostrar login de inmediato sin esperar WS
...
.ia-header::after{
  content:'';position:absolute;bottom:0;top:auto;left:0;right:0;height:3px;
  background:linear-gradient(90deg,#1e3a8a,#2563eb,#60a5fa);
}
.ia-header-logo{
...
const NOVEDADES_DEFAULT_CLIENT=[
...
async function cfgLoadNovedades(){
  try{const r=await fetch('/api/novedades');_novedadesCache=await r.json();}
  catch(e){console.error('cfgLoadNovedades:',e);_novedadesCache=NOVEDADES_DEFAULT_CLIENT;}
}
function cfgGetNovedades(){return _novedadesCache||NOVEDADES_DEFAULT_CLIENT;}
...
setInterval(function(){
  ...
  else { iaRenderTodayChips(); iaUpdateStats(); const _asel=document.getElementById('ia-emp-area'); if(document.activeElement!==_asel) iaBuildAreaSelect(); }
}, 3000);
...
// Fix scroll en selector de módulo en móvil
(function(){
  document.addEventListener('DOMContentLoaded', function(){
    const sel = document.getElementById('ia-emp-area');
    if(!sel) return;
    sel.addEventListener('touchstart', function(e){ e.stopPropagation(); }, {passive:true});
    sel.addEventListener('touchmove', function(e){ e.stopPropagation(); }, {passive:true});
  });
})();
</script>
</body>
