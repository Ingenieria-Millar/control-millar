...
// Delega rutas /bitacora/* a Express, el resto al handler original
...
// ── Rutas Bitácora ────────────────────────────────────────────────
expressApp.use('/bitacora/uploads', express.static(UPLOADS_DIR));

expressApp.get('/bitacora', (req, res) => {
  res.sendFile(path.join(__dirname, 'bitacora.html'));
});
...
// ALISTAMIENTOS
expressApp.get('/bitacora/api/alistamientos', (req, res) => {
...
expressApp.post('/bitacora/api/alistamientos', upload.array('fotos', 5), (req, res) => {
  try {
    const data = loadDB('alistamientos');
    const ahora = new Date();
    const nuevo = { id: uuidv4(), ...req.body,
      fotos: req.files ? req.files.map(f => `/bitacora/uploads/${f.filename}`) : [],
...
expressApp.delete('/bitacora/api/alistamientos/:id', (req, res) => {
...
// MANTENIMIENTOS
expressApp.get('/bitacora/api/mantenimientos', (req, res) => {
...
expressApp.post('/bitacora/api/mantenimientos', upload.array('fotos', 5), (req, res) => {
  try {
...
expressApp.delete('/bitacora/api/mantenimientos/:id', (req, res) => {
...
expressApp.put('/bitacora/api/alertas/leer-todas', (req, res) => {
  const data = loadDB('alertas');
...
// EXPORTAR EXCEL
expressApp.get('/bitacora/api/exportar/alistamientos', (req, res) => {
  const data = loadDB('alistamientos');
...
expressApp.get('/bitacora/api/exportar/mantenimientos', (req, res) => {
  const data = loadDB('mantenimientos');
...
// STATS
expressApp.get('/bitacora/api/stats', (req, res) => {
