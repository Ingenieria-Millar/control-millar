...
// ── Dependencias adicionales para Bitácora ────────────────────────
...
// Delega rutas /alistamiento/* a Express, el resto al handler original
...
function loadDB(key) {
  try {
    if (fs.existsSync(DB_FILES[key])) return JSON.parse(fs.readFileSync(DB_FILES[key], 'utf8'));
  } catch(e) {}
  return [];
}
...
// ── Rutas Bitácora ────────────────────────────────────────────────
expressApp.use('/alistamiento/uploads', express.static(UPLOADS_DIR));

expressApp.get("/alistamiento", (req, res) => {
  res.sendFile(path.join(__dirname, 'alistamientos.html'));
});

expressApp.get('/alistamiento/api/config', (req, res) => {
  res.json({ supervisoras: SUPERVISORAS_BIT, mecanicos: MECANICOS_BIT, modulos: getModulosBit(), maquinas: MAQUINAS_BIT });
});

// ALISTAMIENTOS
expressApp.get('/alistamiento/api/alistamientos', (req, res) => {
...
expressApp.post('/alistamiento/api/alistamientos', upload.array('fotos', 5), (req, res) => {
  try {
    ...
    if (nuevo.pruebaCostura === 'Rechazada') {
      ...
      alertas.push({ id: uuidv4(), tipo:'critica',
...
expressApp.delete('/alistamiento/api/alistamientos/:id', (req, res) => {
  let data = loadDB('alistamientos').filter(r => r.id !== req.params.id);
  saveDB('alistamientos', data);
  res.json({ success: true });
});

// MANTENIMIENTOS
expressApp.get('/alistamiento/api/mantenimientos', (req, res) => {
...
expressApp.post('/alistamiento/api/mantenimientos', upload.array('fotos', 5), (req, res) => {
  try {
    ...
    if (nuevo.tipoMantenimiento === 'Correctivo') {
      const alertas = loadDB('alertas');
      alertas.push({ id: uuidv4(), tipo:'advertencia',
...
expressApp.delete('/alistamiento/api/mantenimientos/:id', (req, res) => {
  let data = loadDB('mantenimientos').filter(r => r.id !== req.params.id);
  saveDB('mantenimientos', data);
  res.json({ success: true });
});

// ALERTAS
expressApp.get('/alistamiento/api/alertas', (req, res) => {
  const data = loadDB('alertas');
  res.json(data.filter(a => !a.leida).sort((a,b) => new Date(b.fechaHora) - new Date(a.fechaHora)));
});
expressApp.put('/alistamiento/api/alertas/:id/leer', (req, res) => {
  const data = loadDB('alertas');
...
expressApp.put('/alistamiento/api/alertas/leer-todas', (req, res) => {
  const data = loadDB('alertas');
  data.forEach(a => a.leida = true);
  saveDB('alertas', data);
  res.json({ success: true });
});

// EXPORTAR EXCEL
expressApp.get('/alistamiento/api/exportar/alistamientos', (req, res) => {
  const data = loadDB('alistamientos');
  const rows = data.map(r => ({'Módulo':r.modulo||'','Referencia':r.referencia||'','Máquina':r.tipoMaquina||'','Serial':r.serial||'','Mecánico':r.mecanico||'','Supervisor':r.supervisor||'','Ficha Técnica':r.fichaTecnica||'','Muestra Física':r.muestraFisica||'','Prueba Costura':r.pruebaCostura||'','Observaciones':r.observaciones||'','Fecha':r.fecha||'','Hora':r.hora||''}));
  const wb = xlsxLib.utils.book_new();
  const ws = xlsxLib.utils.json_to_sheet(rows);
  xlsxLib.utils.book_append_sheet(wb, ws, 'Alistamientos');
...
expressApp.get('/alistamiento/api/exportar/mantenimientos', (req, res) => {
  const data = loadDB('mantenimientos');
  const rows = data.map(r => ({'Módulo':r.modulo||'','Máquina':r.tipoMaquina||'','Serial':r.serial||'','Tipo':r.tipoMantenimiento||'','Repuestos':r.repuestos||'','Mecánico':r.mecanico||'','Observaciones':r.observaciones||'','Fecha':r.fecha||'','Hora':r.hora||''}));
  const wb = xlsxLib.utils.book_new();
  const ws = xlsxLib.utils.json_to_sheet(rows);
  xlsxLib.utils.book_append_sheet(wb, ws, 'Mantenimientos');
  const buf = xlsxLib.write(wb, { type:'buffer', bookType:'xlsx' });
...
// STATS
expressApp.get('/alistamiento/api/stats', (req, res) => {
  const alistamientos = loadDB('alistamientos');
  const mantenimientos = loadDB('mantenimientos');
  const alertas = loadDB('alertas');
  const hoy = new Date().toISOString().split('T')[0];
  res.json({
    alistamientos: { total: alistamientos.length, hoy: alistamientos.filter(r=>r.fecha===hoy).length, aprobados: alistamientos.filter(r=>r.pruebaCostura==='Aprobada').length, rechazados: alistamientos.filter(r=>r.pruebaCostura==='Rechazada').length },
    mantenimientos: { total: mantenimientos.length, hoy: mantenimientos.filter(r=>r.fecha===hoy).length, preventivos: mantenimientos.filter(r=>r.tipoMantenimiento==='Preventivo').length, correctivos: mantenimientos.filter(r=>r.tipoMantenimiento==='Correctivo').length },
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/alistamiento' || req.url.startsWith('/alistamiento/') || req.url.startsWith('/alistamiento?')) {
  ...
  // Control de Piso (index principal)
  ...
  ws.on('message', (raw) => {
    try {
      ...
      // ── Tablero CI: nueva solicitud de insumo ──────────────────
        ...
        const bcast = JSON.stringify({ type: 'ci_new_request', request: msg.request });
        wss.clients.forEach(c => {
          if (c !== ws && c.readyState === 1) c.send(bcast);
        });
      }

      // ── Tablero CI: actualizar solicitud (cumplido, obs) ───────
